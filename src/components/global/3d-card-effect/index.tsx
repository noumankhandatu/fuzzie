"use client";

import Image from "next/image";
import React from "react";
import { CardBody, CardContainer, CardItem } from "./3d-card";
import Link from "next/link";

export function ThreeDCardDemo() {
    return (
        <CardContainer className="inter-var">
            <CardBody className="bg-gray-50 relative group/card dark:hover:shadow-2xl dark:hover:shadow-emerald-500/[0.1] dark:bg-black dark:border-white/[0.2] border-black/[0.1] w-auto sm:w-[30rem] h-auto rounded-xl p-6 border">
                <CardItem
                    translateZ="50"
                    className="text-2xl font-bold text-neutral-800 dark:text-white"
                >
                    Subscription Plan
                </CardItem>
                <CardItem
                    as="p"
                    translateZ="60"
                    className="text-neutral-500 text-base max-w-sm mt-2 dark:text-neutral-300"
                >
                    Choose a plan that fits your needs. Enjoy our services with a 30-day free trial!
                </CardItem>
                <CardItem translateZ="70" className="w-full mt-4">
                    <Image
                        src="https://cdnb.artstation.com/p/assets/images/images/039/110/467/original/art-fume-panda-sleepy-vv.gif?1624977734"
                        height="1000"
                        width="1000"
                        className="h-60 w-full object-cover rounded-xl group-hover/card:shadow-xl"
                        alt="thumbnail"
                    />
                </CardItem>
                <div className="flex flex-col mt-6 space-y-2">
                    <CardItem
                        translateZ="80"
                        className="text-lg font-semibold text-neutral-700 dark:text-white"
                    >
                        Free Trial
                    </CardItem>
                    <CardItem
                        as="p"
                        translateZ="90"
                        className="text-neutral-500 text-sm max-w-sm dark:text-neutral-300"
                    >
                        Start with our 30-day free trial. No credit card required.
                    </CardItem>
                </div>
                <div className="flex justify-between items-center mt-10">
                    <CardItem
                        translateZ="100"
                        as={Link}
                        href=""
                        target="__blank"
                        className="px-4 py-2 rounded-xl text-sm font-normal dark:text-white border dark:border-neutral-300 border-neutral-600"
                    >
                        Learn More
                    </CardItem>
                    <CardItem
                        translateZ="100"
                        as="button"
                        className="px-6 py-2 rounded-xl bg-black dark:bg-white dark:text-black text-white text-sm font-bold"
                    >
                        Sign Up Now
                    </CardItem>
                </div>
            </CardBody>
        </CardContainer>
    );
}
